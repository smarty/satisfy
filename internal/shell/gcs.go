...
func (this *GoogleCloudStorageClient) Upload(request plumbing.UploadRequest) error {
	...
	if err != nil {
		return fmt.Errorf("http error: %s (%w)", err, contracts.ErrRetry)
	}
	...
	if this.isExpectedStatus(response.StatusCode) == false {
		if this.isSafeRetryStatus(response.StatusCode) {
			return fmt.Errorf("http error: %d (%w)", response.StatusCode, contracts.ErrRetry)
		}
...
func (this *GoogleCloudStorageClient) Download(request url.URL) (io.ReadCloser, error) {
	...
	response, err := this.client.Do(gcsRequest)
	if err != nil {
		return nil, fmt.Errorf("http error: %s (%w)", err, contracts.ErrRetry)
	}
	if this.isExpectedStatus(response.StatusCode) == false {
...
func (this *GoogleCloudStorageClient) Size(request url.URL) (int64, error) {
	...
	response, err := this.client.Do(gcsRequest)
	if err != nil {
		return 0, fmt.Errorf("http error: %s (%w)", err, contracts.ErrRetry)
	}
	if this.isExpectedStatus(response.StatusCode) == false {
